...
export async function addProduct(req, res) {
  try {
    ...
    if(!isNaN) return res.status(400).json({error: "Valid Price is required!"});
    ...
    const id = await addProductService({ name, description, sku, price, brand_id, category_id, image_url,image_public_id, stock });
